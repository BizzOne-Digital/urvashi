...
import type { CustomizePrintPlanId } from "./customize-print-plan";
import {
  type CustomizeDesignSetPayload,
  formatCustomizeDesignSetsMessage,
} from "./customize-design-sets";
import {
...
export async function finalizeCustomizeSubmission(
  ...
  const textLines = [
    ...
    "",
    submission.designSets?.length
      ? `Design options submitted: ${submission.designSets.length}`
      : "",
    "",
    ...
    artworkIds.length > 1
      ...
      : artworkIds.length === 1
        ...
        : "No artwork files were attached.",
    "",
    submission.preferDesign
  ...
  await sendEmail({
    ...
    text: [
      ...
      "",
      submission.preferDesign
        ? "We will review your notes and email you 2–3 design options shortly."
        : submission.designSets?.length && submission.designSets.length > 1
          ? `We received your ${submission.designSets.length} design options (front/back) and will contact you about printing.`
...
export async function createCustomizeSubmission(data: {
  ...
  designSets?: CustomizeDesignSetPayload[];
  productSlug?: string;
  productName?: string;
  quantity?: number;
  shipping?: {
  ...
  const designSetsNote = data.designSets?.length
    ? formatCustomizeDesignSetsMessage(data.designSets)
    : "";
  const messageText =
    [designSetsNote, data.message.trim()].filter(Boolean).join("\n\n") ||
    (data.preferDesign
  ...
  const submission = await CustomizeSubmission.create({
    ...
    phone: data.phone,
    message: messageText,
    artworkAssetId: data.artworkAssetIds[0] || undefined,
    artworkAssetIds: data.artworkAssetIds.length ? data.artworkAssetIds : undefined,
    productSlug: data.productSlug,
    ...
    designSets: data.designSets?.length ? data.designSets : undefined,
    baseFee,
    designFee,
...
export async function startCustomizeCheckout(submission: ICustomizeSubmission, totalDue: number) {
  ...
  if ((submission.printSurcharge || 0) > 0) {
    cartItems.push({
      description: "Printing options",
      productCode: "PRINT",
      unitCost: submission.printSurcharge || 0,
      quantity: 1,
    });
  }
  ...
  const ticket = await monerisPreload({
    ...
    cartTax:
      submission.tax > 0
        ? { amount: submission.tax, description: submission.taxLabel || "Tax" }
        : undefined,
  });
