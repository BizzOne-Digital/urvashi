...
export default async function ProductDetailPage({ params }: Props) {
  ...
  <>
        ...
        <Container>
          ...
          <div className="grid gap-12 lg:grid-cols-2">
            ...
            <div>
              ...
              <h1 className="heading-section mt-2">{product.name}</h1>
              {product.shortDescription && (
                <p className="mt-4 text-carbon">{product.shortDescription}</p>
              )}
              <AddToCartForm product={{ ...product, _id: String(product._id) }} className="mt-8" />
            </div>
          </div>
        </Container>
      </section>
      ...
      <section className="border-t border-chrome-light/40 py-12">
        <Container>
          <Link href="/shop" className={buttonVariants("secondary")}>← Back to shop</Link>
        </Container>
      </section>
    </>
  );
