    ...
    <>
      ...
      <section className="py-20">
        <Container className="grid items-center gap-12 lg:grid-cols-2">
          <div>
            <h2 className="heading-section">About DPM Custom Prints</h2>
            <p className="mt-4 text-carbon leading-relaxed">
          ...
          <div className="grid grid-cols-2 gap-4">
            {DEMO_IMAGES.slice(0, 6).map((src) => (
              <div key={src} className="relative aspect-square overflow-hidden rounded-sm border border-chrome-light/60">
                <Image src={src} alt="About DPM sample print" fill className="object-contain p-4" sizes="25vw" />
              </div>
            ))}
          </div>
        </Container>
        ...
        <Container>
          ...
          <div className="mt-10 grid gap-8 md:grid-cols-3">
            {[
              ...
              <div key={item.title} className="rounded-sm border border-chrome-mid/30 p-6">
                <div className="relative mb-4 aspect-video overflow-hidden rounded-sm bg-carbon">
                  <Image src={item.img} alt={item.title} fill className="object-contain p-4" sizes="33vw" />
                </div>
      ...
      <section className="py-20">
        <Container className="text-center">
          ...
          <p className="mx-auto mt-4 max-w-xl text-carbon">Browse our shop, open the customizer, or reach out for a consultation.</p>
          <div className="mt-8 flex flex-wrap justify-center gap-4">
            <Link href="/shop" className={buttonVariants("primary")}>Shop products</Link>
            <Link href="/contact" className={cn(buttonVariants("secondary"))}>Contact us</Link>
          </div>
        </Container>
      </section>
    </>
