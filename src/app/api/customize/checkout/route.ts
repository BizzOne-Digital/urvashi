...
import { customizePrintPlanSchema, formatPrintPlanOrderNote } from "@/lib/customize-print-plan";
import { flattenDesignSetArtworkIds } from "@/lib/customize-design-sets";

const designSetSchema = z.object({
  printPlan: customizePrintPlanSchema,
  frontArtworkAssetIds: z.array(z.string().min(1)).optional(),
  backArtworkAssetIds: z.array(z.string().min(1)).optional(),
...
const checkoutSchema = z
  .object({
    ...
    printPlan: customizePrintPlanSchema.default("print_wrap"),
    designSets: z.array(designSetSchema).max(3).optional(),
  ...
  .refine(
    (data) => {
      if (data.preferDesign) return true;
      const fromSets = flattenDesignSetArtworkIds(data.designSets || []);
      const fromFlat =
        (data.artworkAssetIds?.length || 0) > 0 || Boolean(data.artworkAssetId);
      return data.printPlan === "blank" || fromSets.length > 0 || fromFlat;
    },
    {
      message: "At least one artwork upload is required for this print option",
...
export async function POST(request: NextRequest) {
  try {
    ...
    const artworkFromSets = flattenDesignSetArtworkIds(data.designSets || []);
    const artworkAssetIds = artworkFromSets.length
      ? artworkFromSets
    ...
    const printPlanNote =
      data.designSets?.length ? "" : formatPrintPlanOrderNote(data.printPlan);
    const message = [printPlanNote, data.message || ""].filter(Boolean).join("\n\n");
