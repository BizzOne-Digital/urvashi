...
export function BookingForm({ methods = [], className }: BookingFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className={cn("space-y-5", className)} noValidate>
      ...
      <div>
        <label htmlFor="requestType" className="mb-1 block text-sm font-medium">Consultation type</label>
        <select id="requestType" {...register("requestType")} className={fieldClass}>
          {requestTypes.map((t) => (
      ...
      <div className="grid gap-5 sm:grid-cols-3">
        <div>
          <label htmlFor="preferredDate" className="mb-1 block text-sm font-medium">Preferred date</label>
          <input id="preferredDate" type="date" {...register("preferredDate")} className={fieldClass} />
        </div>
        <div>
          <label htmlFor="preferredTimeWindow" className="mb-1 block text-sm font-medium">Time window</label>
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        <div>
          <label htmlFor="productInterest" className="mb-1 block text-sm font-medium">Product interest</label>
          <input id="productInterest" {...register("productInterest")} className={fieldClass} />
        </div>
        <div>
          <label htmlFor="approximateQuantity" className="mb-1 block text-sm font-medium">Approximate quantity</label>
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        <div>
          <label htmlFor="eventDate" className="mb-1 block text-sm font-medium">Event date (optional)</label>
          <input id="eventDate" type="date" {...register("eventDate")} className={fieldClass} />
        </div>
        <div>
          <label htmlFor="flexibility" className="mb-1 block text-sm font-medium">Schedule flexibility</label>
          <input id="flexibility" placeholder="e.g. Weekdays only" {...register("flexibility")} className={fieldClass} />
        </div>
      </div>

      <div>
        <label htmlFor="message" className="mb-1 block text-sm font-medium">Additional details</label>
