...
export function ContactForm({ className, serviceOptions = ["Custom printing"] }: ContactFormProps) {
  ...
  const fieldClass =
    "w-full rounded-sm border border-white/15 bg-white/5 px-4 py-3 text-sm text-pure-paper placeholder:text-chrome-mid focus:border-cyan/50 focus:outline-none focus:ring-2 focus:ring-cyan/25";
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className={cn("min-w-0 space-y-5", className)} noValidate>
      ...
      <div>
        <label htmlFor="inquiryType" className="mb-1 block text-sm font-medium text-pure-paper">Inquiry type</label>
        <select id="inquiryType" {...register("inquiryType")} className={cn(fieldClass, "[&>option]:bg-ink-black [&>option]:text-pure-paper")}>
          {INQUIRY_TYPES.map((t) => (
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        <div>
          <label htmlFor="productInterest" className="mb-1 block text-sm font-medium text-pure-paper">Product interest (optional)</label>
          <input id="productInterest" {...register("productInterest")} className={fieldClass} />
        </div>
        <div>
          <label htmlFor="serviceInterest" className="mb-1 block text-sm font-medium text-pure-paper">Service interest (optional)</label>
          <select id="serviceInterest" {...register("serviceInterest")} className={cn(fieldClass, "[&>option]:bg-ink-black [&>option]:text-pure-paper")}>
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        <div>
          <label htmlFor="quantity" className="mb-1 block text-sm font-medium text-pure-paper">Approximate quantity (optional)</label>
          <input id="quantity" type="number" min={1} {...register("quantity")} className={fieldClass} />
        </div>
        <div>
          <label htmlFor="deadline" className="mb-1 block text-sm font-medium text-pure-paper">Deadline (optional)</label>
          <input id="deadline" type="date" {...register("deadline")} className={fieldClass} />
        </div>
      </div>

      <div>
        <label htmlFor="message" className="mb-1 block text-sm font-medium text-pure-paper">Message</label>
